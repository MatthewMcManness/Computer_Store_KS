/**
 * Admin catalog sync trigger API route (Phase 3 stub).
 *
 * POST: Triggers a catalog sync from TD Synnex. This is a stub
 * for Phase 3 implementation. Currently returns a "not yet configured"
 * message.
 *
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Phase 3: TD Synnex catalog sync will be implemented here
    return NextResponse.json({
      success: true,
      message: 'Catalog sync not yet configured. TD Synnex integration pending (Phase 3).',
      sync_type: 'catalog',
      status: 'not_configured',
    });
  } catch (error) {
    console.error('Error in POST /api/admin/store/sync/catalog:', error);
    return NextResponse.json(
