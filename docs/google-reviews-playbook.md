...
| 2026-05-18 | Local login → Supabase OAuth | Login button worked, but after Google sign-in the browser landed on `https://computerstoreks.com` instead of `http://localhost:3000/admin`. Visiting `/api/google-business/oauth/start` then returned `{"error":"Unauthorized"}`. | Supabase silently ignores the `redirectTo` option unless the URL matches an entry in **Authentication → URL Configuration → Redirect URLs** and falls back to **Site URL** instead. The project's Supabase only allowlisted production, so the session cookie was set on `computerstoreks.com` and localhost saw an unauthenticated request. | Add `http://localhost:3000/**` to **Redirect URLs** in the Supabase dashboard. `Site URL` stays as production. After saving, the local login flow lands on `localhost:3000/admin` and the admin-gated API routes accept the local session cookie. |
| 2026-05-18 | Production OAuth connect flow | Got redirected to `/admin?gbp=exchange_failed` after granting consent. Render logs showed `[oauth-callback] failed: Error: Google API request failed (429) https://mybusinessaccountmanagement.googleapis.com/v1/accounts?pageSize=50` | The OAuth grant itself succeeded (token exchange + Supabase write both completed). Failure was in the cache-seed step calling `accounts.list`. Google returns **429 Too Many Requests** for projects that have not yet cleared the GBP API allowlist — even on the very first call. It's a policy gate, not a real quota event. Our original callback used one try/catch and reported any failure as `exchange_failed`, which made the error look like a credentials issue. | Two fixes: (a) split the callback into three try/catch blocks so the three failure modes (`exchange_failed`, `db_failed`, `pending`) are distinguishable from the URL alone. The `pending` outcome means the grant is persisted and only the cache seed failed — a manual `POST /api/google-business/refresh` will succeed once Google approves. (b) Document that 429 on first call ≈ "still waiting on allowlist." |
